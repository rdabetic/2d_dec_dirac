...
int main(int argc, char* argv[]) {
  ...
  auto f1 = [](const Eigen::Vector2d& v) -> Eigen::Vector2d {
    ...
    return Eigen::Vector2d({
        0,
        4 * 2 * M_PI * std::sin(2 * 2 * M_PI * x) * std::cos(2 * 2 * M_PI * y)
    });
  };
  auto f2 = [](const Eigen::Vector2d v) -> double {
    const double& x = v(0),
                  y = v(1);
    return 2 * 2 * M_PI * (std::cos(2 * 2 * M_PI * x) - std::cos(2 * 2 * M_PI * y));
  };
  // The exact solution
  auto u0 = [](const Eigen::Vector2d& v) -> double {
    const double& x = v(0),
                  y = v(1);
    return std::sin(2 * 2 * M_PI * x) * std::sin(2 * 2 * M_PI * y);
  };
  ...
  auto u2 = [](const Eigen::Vector2d& v) -> double {
    const double& x = v(0),
                  y = v(1);
    return std::cos(2 * 2 * M_PI * x) * std::cos(2 * 2 * M_PI * y);
  };
  // Mesh
